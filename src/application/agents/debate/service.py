...
from src.presentation.api.schemas import PersonaConfig, AgentMetadata
...
# Default debate settings (from config)
DEFAULT_MAX_ROUNDS = settings.debate_max_rounds
DEFAULT_STABILITY_THRESHOLD = settings.debate_stability_threshold
DEFAULT_CONSECUTIVE_STABLE_ROUNDS = settings.debate_consecutive_stable_rounds

# Valid persona styles for debate agents
VALID_PERSONA_STYLES = {"critical", "supportive", "neutral"}


def _get_style_category(style: Optional[str]) -> str:
    """Helper to categorize style string into one of the three main categories."""
    if not style:
        return "Neutral"
...
def generate_system_prompt(persona: PersonaConfig) -> str:
    ...
    style_category = _get_style_category(persona.style)

    if style_category == "Critical":
    ...
    elif style_category == "Supportive":
...
def get_few_shot_example(persona: PersonaConfig) -> str:
    ...
    if persona.corpus:
        ...
        verdict_map = {"Critical": 0, "Supportive": 2, "Neutral": 1}
        verdict = verdict_map.get(style_category, 1)
...
{{
    ...
    style_category = _get_style_category(persona.style)
    return FEW_SHOT_EXAMPLES.get(style_category, FEW_SHOT_EXAMPLES["Neutral"])
...
class DebateStatistics:
    ...
    def add_round(
        ...
        if num_votes > 0:
            ...
            consensus_level = majority_count / num_votes
            # Calculate vote variance and mean
            vote_variance = float(np.var(votes))
            vote_mean = float(np.mean(votes))
        else:
            consensus_level = 0.0
            vote_variance = 0.0
            vote_mean = 0.0
        ...
        for i, agent_id in enumerate(agent_ids):
            ...
            # Handle case where votes might be shorter than agent_ids (though unlikely in valid flow)
            vote_val = votes[i] if i < len(votes) else 0
            self.agent_vote_history[agent_id].append(vote_val)
        ...
        if len(self.round_stats) >= 2:
            ...
            try:
                # Only calculate if both rounds have votes
                if prev_votes and votes:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        # final_votes = final_round["votes"]  # Unused variable
    ...
    def _detect_convergence(self) -> Tuple[bool, Optional[int]]:
        ...
        """
        history_len = len(self.convergence_history)
        if history_len < DEFAULT_CONSECUTIVE_STABLE_ROUNDS:
            return False, None
        ...
        # We look backwards from the start of the stable window
        start_check_idx = history_len - DEFAULT_CONSECUTIVE_STABLE_ROUNDS
        convergence_round = None

        # Check backwards to find the first stable point
        for i in range(start_check_idx, -1, -1):
             if self.convergence_history[i] >= DEFAULT_STABILITY_THRESHOLD:
                # The round AFTER the last unstable one is the start of convergence
                convergence_round = i + DEFAULT_CONSECUTIVE_STABLE_ROUNDS + 1
                break
        
        if convergence_round is None:
            # If we went all the way back and everything was stable (unlikely but possible)
            # or if the loop didn't break (meaning all checked were stable)
            # The logic here is slightly tricky. If we never found an unstable round,
            # it means stability started early.
            # Let's simplify: if the loop finishes without break, it means all checked history was stable?
            # No, the loop goes backwards. If we don't find a value >= threshold, it means
            # all values from 0 to start_check_idx are < threshold.
            # So convergence started at the beginning of our tracking.
            # Since convergence_history[0] corresponds to round 2 (comparison of round 1 and 2),
            # let's just say round 2.
            convergence_round = 2
...
class PersonaGenerator:
    ...
    def _get_corpus_text(self, corpus: Optional[List[str]], limit: int = 5, prefix: str = "\n\nUser's previous statements:") -> str:
        """Helper to format corpus text."""
        if corpus:
            return f"{prefix}\n{chr(10).join(f'- {statement}' for statement in corpus[:limit])}"
        return ""
...
- An appropriate debate style"""
    ...
    async def _detect_user_style(self, corpus: List[str]) -> Optional[str]:
        ...
        try:
            ...
            corpus_text = self._get_corpus_text(sample_corpus, limit=10, prefix="User statements:")
...
class DebateService:
    ...
    def create_session(
        ...
        Args:
            ...
            corpus: Optional user history statements for style detection and few-shot examples
            detected_user_style: Optional detected user style ("Critical", "Supportive", "Neutral")
    ...
    @cached(ttl=3600)  # Cache LLM responses for 1 hour
    ...
    async def execute_round_reasoning(self, session_id: str) -> Dict[str, Any]:
        ...
        async def _process_single_agent(agent):
            try:
                return await self._execute_agent_reasoning(agent, session["topic"], debate_context)
            except Exception as e:
                logger.error(f"Agent {agent.agent_id} reasoning failed: {e}")
                return e

        reasoning_results = await asyncio.gather(*[_process_single_agent(a) for a in agents], return_exceptions=True)
        ...
        for i, result in enumerate(reasoning_results):
            ...
            if isinstance(result, Exception):
                ...
                reasoning = result
                # Ensure result is a dict before assignment
                if not isinstance(reasoning, dict):
                    logger.error(f"Agent {agent.agent_id} returned non-dict reasoning: {reasoning}")
                    reasoning = {
                        "statement": "Error processing response.",
                        "reasoning": "Invalid response format."
                    }

                reasoning["agent_id"] = agent.agent_id
                reasoning["timestamp"] = timestamp
                reasoning["word_count"] = len(reasoning.get("statement", "").split()) + len(reasoning.get("reasoning", "").split())

            round_reasonings.append(reasoning)
            total_word_count += reasoning.get("word_count", 0)
    ...
    async def execute_round_voting(self, session_id: str) -> Dict[str, Any]:
        ...
        async def _process_single_agent_vote(agent):
            try:
                return await self._execute_agent_voting(agent, session["topic"], voting_context)
            except Exception as e:
                logger.error(f"Agent {agent.agent_id} voting failed: {e}")
                return e

        voting_results = await asyncio.gather(*[_process_single_agent_vote(a) for a in agents], return_exceptions=True)
        ...
        for i, result in enumerate(voting_results):
            ...
            if isinstance(result, Exception):
                ...
                if not isinstance(vote, dict):
                    logger.error(f"Agent {agent.agent_id} returned non-dict vote: {vote}")
                    vote = {
                        "verdict": 1,
                        "reasoning": "Invalid response format."
                    }
                ...
                vote["word_count"] = len(vote.get("reasoning", "").split())

            round_votes.append(vote)
            total_word_count += vote.get("word_count", 0)
...
{agent.few_shot_example}"""
    ...
    def clear_cache(self) -> None:
        """
        Clear in-memory cache and statistics.

        This method provides a public interface to clear the internal
        cache and statistics, maintaining encapsulation.
        """
        self._sessions_cache.clear()
        self._statistics.clear()
        logger.info("DebateService cache cleared")
