...
class BotCreatorAgent:
    ...
    def __init__(
        ...
        if settings.enable_persistence:
            try:
                database = get_database()
                self._custom_few_shots = database.load_custom_few_shots("bot_creator")
...
5. Example interactions or use cases""")
    ...
    def create_bot(
        ...
        try:
            ...
            bot_name_final = bot_name or f"bot_{len(self.created_bots) + 1}"
            bot_entry = {
            ...
            logger.info(f"Bot created successfully: bot_name={bot_entry['bot_name']}, mode={self.persona_mode}, execution_mode={execution_mode}")

            if self.vector_store:
                logger.debug(f"Storing bot {bot_name_final} configuration in vector memory")
                self._store_in_memory(persona_prompt, bot_configuration, bot_name_final)
...
"""
    ...
    def get_bot(self, bot_name: str) -> Optional[Dict[str, Any]]:
        """Retrieve a created bot by its name."""
    ...
    def chat_with_bot(
        ...
        Args:
            bot_name: The name of the bot to chat with
        ...
        logger.info(f"Chat request for bot {bot_name}: message length={len(user_message)}, history_mode={history_mode}")
        ...
        try:
            ...
            logger.error(f"Failed to chat with bot {bot_name}: {e}", exc_info=True)
            return {
    ...
    def chat_with_bot_by_config(
        ...
        """
        Chat with a bot using its configuration.
    ...
    def _store_in_memory(self, persona_prompt: str, bot_config: str, bot_name: str) -> None:
        """Store the bot configuration in vector memory."""
        if not self.vector_store:
            return

        doc_text = f"Bot Name: {bot_name}\nPersona Prompt:\n{persona_prompt}\n\nBot Configuration:\n{bot_config}"
        metadata = {
