...
(Media Bias/Fact Check) persistence support as optional prior probability.

Core Philosophy:
- "Glass Box" Design: Every agent/step is a public method for debugging
- MBFC is Optional: Works with or without persistence (use_mbfc=False)
...
from src.presentation.api.schemas import BiasDistribution
from src.shared.cache import cached
from src.shared.config.settings import settings
from src.shared.constant.enums import LogicMode
from src.shared.constant.exceptions import NetworkError, ContentExtractionError
...
# NOTE: LLM retry is now handled by @llm_retry decorator from shared.llm
# Network retry has been removed - only LLM calls use retry as per requirements
...
class WebOpinionEngine:
    ...
    Agent 2: resolve_metadata(url) -> SourceMetadata
        - Optional MBFC persistence lookup (controlled by use_mbfc parameter)
    ...
    def __init__(
        ...
        Args:
            ...
            db_path: Path to MBFC SQLite persistence (optional)
        ...
        self.max_chunk_tokens = max_chunk_tokens or settings.web_opinion_max_chunk_tokens
    ...
    def set_custom_atomizer_shots(cls, custom_shots: Optional[List[dict]] = None) -> None:
        ...
        if settings.enable_persistence:
            ...
            if database:
                success = database.save_custom_few_shots("web_opinion_extractor_atomizer", json.dumps(custom_shots))
                if success:
                    logger.info(f"Custom atomizer shots saved to persistence: {custom_shots is not None}")
                else:
                    logger.warning("Failed to save custom atomizer shots to persistence")
        else:
    ...
    def set_custom_scorer_shots(cls, custom_shots: Optional[List[dict]] = None) -> None:
        ...
        if settings.enable_persistence:
            database = get_database()
            if database:
                success = database.save_custom_few_shots("web_opinion_extractor_scorer", json.dumps(custom_shots))
                if success:
                    logger.info(f"Custom scorer shots saved to persistence: {custom_shots is not None}")
                else:
                    logger.warning("Failed to save custom scorer shots to persistence")
        else:
    ...
    def get_custom_atomizer_shots(cls) -> Optional[List[dict]]:
        ...
        # If not loaded yet, try to load from persistence
        if cls._custom_atomizer_shots is None and settings.enable_persistence:
            ...
            if database:
                ...
                if isinstance(stored_shots, list):
                    ...
                    logger.info("Loaded custom atomizer shots from persistence")
        ...
        if cls._custom_scorer_shots is None and settings.enable_persistence:
            ...
            if database:
                ...
                if isinstance(stored_shots, list):
                    ...
                    logger.info("Loaded custom scorer shots from persistence")
    ...
    def _get_db_connection(self):
        """Context manager for SQLite persistence connections."""
        if not self.db_path:
            yield None
            return
    ...
    def resolve_metadata(self, url: str) -> SourceMetadata:
        """
        Agent 2: Resolve source metadata from MBFC persistence (optional).
        
        Logic:
        1. Extract and normalize domain from URL
        2. Query persistence: SELECT * FROM media_sources WHERE source_url LIKE %domain%
        3. Smart matching:
        ...
        if not effective_db_path:
            logger.info("No persistence path configured, returning disabled metadata")
        ...
        with self._get_db_connection() as conn:
            if conn is None:
                logger.warning("Database persistence failed, returning none metadata")
                return SourceMetadata(
                    source_name=None,
                    raw_db_row=None,
                    match_type="none",
            ...
            try:
                ...
                if len(rows) == 0:
                    # No match
                    logger.info(f"No persistence match for domain: {domain}")
                    return SourceMetadata(
                        source_name=None,
                        raw_db_row=None,
                        match_type="none",
...
Return JSON only."""
    ...
    def run(
        ...
        **kwargs
        ...
        Args:
            ...
            use_mbfc: Whether to use MBFC persistence for prior probability (default: True)
            use_few_shots: Whether to use few-shot examples (default: True)
    ...
    def run_pipeline(
        ...
        Args:
            url: URL to analyze
            use_mbfc: Whether to use MBFC persistence (default: True)
