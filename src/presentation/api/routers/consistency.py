...
async def _check_summary_vs_url_consistency(summary: str, url: str, enable_deep_analysis: bool = True, similarity_threshold: float = 0.2,
    ...
    try:
        ...
        # Note: extract_from_url might be synchronous. If so, it blocks.
        # Assuming it's synchronous for now, but ideally should be async or run in executor.
        # For this optimization, we'll focus on the LLM parts which are definitely blocking if not awaited.
        ...
        try:
            ...
            if not summary_texts or not url_texts:
                ...
                similarity_matrix = np.zeros((len(summary_texts), len(url_texts)))
            else:
                all_texts = summary_texts + url_texts

                # Get embeddings (async if possible, otherwise sync)
                # OpenAIEmbeddings.aembed_documents is available in newer versions
        ...
        async def analyze_pair(summary_claim, url_claim, similarity_score):
            # If deep analysis is disabled, skip LLM call and return based purely on similarity
            ...
            try:
                # Adjust prompt based on CoT mode
                if use_cot_audit == CoTMode.CHAIN_ONLINE:
                    system_prompt = """You are an expert fact-checker. Compare the following claims step-by-step.
1. Analyze the meaning of the summary claim.
2. Analyze the meaning of the URL claim.
3. Determine if they refer to the same fact/event.
4. If they do, check if the URL claim supports or contradicts the summary claim.
...
STATUS: [supported|contradicted|neutral]
CONFIDENCE: [0.0-1.0]
REASON: [Concise explanation based on thoughts]"""
                else:
...
Similarity score: {similarity_score:.3f}"""
                ...
                return {
                    "summary_claim": {"id": summary_claim.id, "text": summary_claim.text},
                    "url_claim": {"id": url_claim.id, "text": url_claim.text},
                    "similarity_score": float(similarity_score),
                    "relationship": "error",
                    "confidence": 0.0,
